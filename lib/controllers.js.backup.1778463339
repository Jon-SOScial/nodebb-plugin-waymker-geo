...
// Approximate centroids for ZIP codes - randomized within ZIP bounds
function getApproximateLocation(zipCode, latitude, longitude) {
	// For regular users, create a randomized location within ~5-7 miles of actual location
	// This creates a consistent but obfuscated position per user
	if (latitude && longitude) {
		const lat = parseFloat(latitude);
		const lng = parseFloat(longitude);
		
		// Use ZIP code as seed for consistent randomization
		const seed = zipCode ? zipCode.charCodeAt(0) + zipCode.charCodeAt(zipCode.length - 1) : 0;
		const randomFactorLat = ((seed * 12.9898) % 1) * 0.08 - 0.04; // ~4-5 miles variation
		const randomFactorLng = ((seed * 78.233) % 1) * 0.08 - 0.04;  // ~4-5 miles variation
		
		return {
			latitude: lat + randomFactorLat,
			longitude: lng + randomFactorLng,
			isApproximate: true,
			approximateLevel: zipCode ? 'ZIP Code' : 'City'
		};
	}
	...
	try {
		...
		const results = userFields
			...
			.map((u) => {
				...
				const lat = parseFloat(u['waymkerGeo:latitude']);
				const lng = parseFloat(u['waymkerGeo:longitude']);
				const zipCode = u['waymkerGeo:zipCode'] || '';
				
				// For regular users, ALWAYS use approximate location
				// For privileged users, use exact coordinates
				let displayLat = lat;
				let displayLng = lng;
				let isApproximate = false;
				let approximateLevel = null;
				
				if (!isPrivileged) {
					// Regular users ALWAYS see approximate location
					const approxLocation = getApproximateLocation(zipCode, lat, lng);
					if (approxLocation) {
						displayLat = approxLocation.latitude;
						displayLng = approxLocation.longitude;
						isApproximate = true;
						approximateLevel = approxLocation.approximateLevel;
					}
				}
				
				// Calculate distance from caller's exact location to user's display location
				const distance = haversineDistance(callerLat, callerLng, displayLat, displayLng);
				...
				const userData = {
					...
					zipCode: u['waymkerGeo:zipCode'] || '',
					roles: userGroupMap[uid] || [],
					// Map display coordinates (approximate for regular users, exact for privileged)
					mapLatitude: displayLat,
					mapLongitude: displayLng,
					isApproximate: isApproximate,
					approximateLevel: approximateLevel,
				};

				// Only privileged users see exact coordinates and address
				if (isPrivileged) {
		...
		logDebug('Query: roleFilter=' + roleFilter + ' groupFilter=' + groupFilter + ' Results=' + results.length + ' isPrivileged=' + isPrivileged);
