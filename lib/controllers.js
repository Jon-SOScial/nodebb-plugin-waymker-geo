...
// Approximate centroids for US neighborhoods/ZIP codes
// This is a simplified mapping - in production you'd use a proper geocoding service
function getApproximateLocation(neighborhood, zipCode, city, state, latitude, longitude) {
	// If we have exact coordinates, use them (for privileged users)
	if (latitude && longitude) {
		return { latitude: parseFloat(latitude), longitude: parseFloat(longitude), isExact: true };
	}
	
	// For regular users, create an approximate location based on neighborhood/ZIP
	// Add some randomization within the ZIP code bounds (~2-3 miles)
	if (zipCode || neighborhood) {
		const randomOffset = () => (Math.random() - 0.5) * 0.04; // ~2-3 miles
		const approximateLat = parseFloat(latitude) + randomOffset();
		const approximateLng = parseFloat(longitude) + randomOffset();
		
		return {
			latitude: approximateLat,
			longitude: approximateLng,
			isApproximate: true,
			approximateLevel: zipCode ? 'ZIP Code' : 'Neighborhood'
		};
	}
	...
	try {
		...
		const results = userFields
			...
			.map((u) => {
				...
				// For regular users, use approximate coordinates based on neighborhood/ZIP
				let displayLat = lat;
				let displayLng = lng;
				...
				if (!isPrivileged) {
					// Regular users see approximate location
					const approxLocation = getApproximateLocation(
						u['waymkerGeo:neighborhood'],
						u['waymkerGeo:zipCode'],
						u['waymkerGeo:city'],
						u['waymkerGeo:state'],
						lat,
						lng
					...
					isApproximate = approxLocation.isApproximate;
				...
				// Only privileged users see exact coordinates
				if (isPrivileged) {
