...
async function getFollowingList(uid) {
	return new Promise((resolve) => {
		...
		user.getUserField(uid, 'username', (err, username) => {
			if (err || !username) {
				logDebug('Error getting username for uid ' + uid);
				resolve([]);
				return;
			}

			const baseUrl = nconf.get('url');
			const urlStr = baseUrl + '/api/user/' + username + '/following';
			const protocol = urlStr.startsWith('https') ? https : http;
			
			logDebug('Fetching following from: ' + urlStr);
			...
			protocol.get(urlStr, (res) => {
				...
				res.on('end', () => {
					try {
						...
						const followingUsers = Array.isArray(json) ? json : (json.users || []);
						const followingUids = followingUsers.map((u) => {
							if (typeof u === 'object' && u.uid) return parseInt(u.uid, 10);
							if (typeof u === 'number') return parseInt(u, 10);
							return null;
						}).filter(id => id !== null);
						logDebug('Got ' + followingUids.length + ' following users for uid ' + uid + ': ' + JSON.stringify(followingUids));
						resolve(followingUids);
					} catch (e) {
				...
				logDebug('Error fetching following from ' + urlStr + ': ' + e.message);
				resolve([]);
			});
		});
	});
...
exports.getUsersNearMe = async function (req, res) {
	try {
		...
		// Get users this caller is following
		let callerFollowing = [];
		const followingFilter = req.query.following || 'all';
		if (followingFilter !== 'all') {
			callerFollowing = await getFollowingList(callerUid);
		}
