...
exports.getUsersNearMe = async function (req, res) {
	try {
		...
		// Get caller's friends - try multiple key formats
		let callerFriends = [];
		try {
			...
			console.log('[waymker-geo] Caller', callerUid, 'friends key:', friendsKey, 'friends:', callerFriends);
		} catch (e) {
			console.warn('[waymker-geo] Could not fetch caller friends:', e);
		}
		...
		try {
			...
			console.warn('[waymker-geo] Could not fetch user groups:', e);
		}
		...
		const results = userFields
			...
			.filter((u) => {
				...
				// Check if user is in system groups for roles
			...
			.filter((u) => {
				...
				// Filter by custom group membership
			...
			.filter((u) => {
				...
				console.log('[waymker-geo] User', uid, 'isFriend?', isFriend, 'in array:', callerFriends);
				if (friendsFilter === 'friends') {
					return isFriend;
				}
				if (friendsFilter === 'nonfriends') {
					return !isFriend;
				}
				return true;
			})
		...
		console.log('[waymker-geo] Query - roleFilter:', roleFilter, 'groupFilter:', groupFilter, 'friendsFilter:', friendsFilter, 'Results:', results.length);
