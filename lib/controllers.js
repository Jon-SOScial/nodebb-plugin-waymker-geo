...
exports.getUsersNearMe = async function (req, res) {
	try {
		...
		let callerFollowing = [];
		try {
			let followingKey = 'uid:' + callerUid + ':following';
			callerFollowing = await db.getSetMembers(followingKey);
			callerFollowing = callerFollowing.map((f) => parseInt(f, 10));
			logDebug('Caller ' + callerUid + ' is following: ' + JSON.stringify(callerFollowing));
		} catch (e) {
