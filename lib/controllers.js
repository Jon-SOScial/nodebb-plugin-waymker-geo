...
exports.getUsersNearMe = async function (req, res) {
	try {
		...
		const rolesFilter = req.query.roles ? req.query.roles.split(',').map((r) => r.trim()).filter(Boolean) : [];
		...
		const results = userFields
			...
			.filter((u) => {
				...
				return rolesFilter.some((r) => u.roles && u.roles.indexOf(r) !== -1);
			})
			.filter((u) => {
