...
exports.getUsersNearMe = async function (req, res) {
	try {
		...
		if (followingFilter !== 'all') {
			try {
				callerFollowing = await db.getSortedSetRange('uid:' + callerUid + ':following', 0, -1);
				callerFollowing = callerFollowing.map((f) => parseInt(f, 10));
				logDebug('Got following via sorted set for uid ' + callerUid + ': ' + JSON.stringify(callerFollowing));
			} catch (e) {
				logDebug('Error getting following: ' + e.message);
			}
		}
		...
		const results = userFields
			...
			.filter((u) => {
				...
				logDebug('Following check: ' + u.username + ' uid=' + uid + ' isFollowing=' + isFollowing + ' callerFollowing=' + JSON.stringify(callerFollowing));
