...
exports.getUsersNearMe = async function (req, res) {
	try {
		...
		try {
			...
			console.log('[waymker-geo] Caller UID', callerUid, 'has', callerFriends.length, 'friends:', callerFriends);
		} catch (e) {
		...
		try {
			for (const uid of uids) {
				...
				if (userGroups && userGroups.length > 0) {
					console.log('[waymker-geo] User', uid, 'groups:', userGroups.map(g => typeof g === 'string' ? g : g.name));
				}
				userGroupMap[uid] = userGroups || [];
			}
		} catch (e) {
		...
		const results = userFields
			...
			.filter((u) => {
				...
				if (friendsFilter === 'friends') {
					...
					console.log('[waymker-geo] User', uid, 'is friend?', isFriend, 'callerFriends:', callerFriends);
					return isFriend;
				}
