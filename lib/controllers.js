...
exports.getUsersNearMe = async function (req, res) {
	try {
		...
		let callerFollowing = [];
		try {
			const userdata = await user.getUserData(callerUid);
			if (userdata && userdata.following && Array.isArray(userdata.following)) {
				callerFollowing = userdata.following.map((f) => parseInt(f, 10));
				logDebug('Caller ' + callerUid + ' is following (via API): ' + JSON.stringify(callerFollowing));
			} else {
				logDebug('No following data found for user ' + callerUid);
			}
		} catch (e) {
			logDebug('Error getting following data: ' + e.message);
		}
